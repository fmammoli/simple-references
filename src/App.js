...
function AbntItem({ reference }) {
  ...
  function formatReference(reference) {
    const authorsString = `${reference.authors
      .flatMap((item) => `${item.family.toUpperCase()}, ${item.given}`)
      .join("; ")}.`;
    ...
    const monthString = reference.month
      ? `${months[reference.month - 1]}.`
      : "";
    const yearString = reference.year ? `${reference.year}.` : "";
    const availableAtSring = reference.availableAt
      ? `Disponível em: <${reference.availableAt}>`
      : "";
  ...
  return <p>{formatReference(reference)}</p>;
}
...
function SearchItem({ item, setDOI, setSearchList }) {
  ...
  return (
    <li>
      ...
      <p>
        Author: {item.author?.[0].given} {item.author?.[0]?.family}
      </p>
...
function App() {
  ...
  const [
    { data: doiData, isLoading: doiIsLoading, isError: doiIsError },
    setDOI,
  ] = useDOITextResponse(null);

  //const [{data, isLoading, isError}, setUrl] = useFetch('', '');

  const [searchList, setSearchList] = React.useState([]);
  const [
    { data: searchData, isLoading: searchIsLoading, isError: searchIsError },
    setSearchQuery,
  ] = useFreeSearch(null);
  ...
  return (
    <main className="App">
      ...
      <section>
        <p>Input a DOI and get a ABNT formatted reference.</p>
        <p>* It only works for journal articles yet*</p>
        <p>
          This uses simple content negotiation to retrieve metadata from DOIs.
        </p>
        <p>
          You can search by text in the Free Text Search. Text searches uses the
          CrossRef API, so it works better for journal articles than books.
        </p>
        <div>
